...
## 收尾铁律：大方案执行完必须复盘并沉淀（2026-09-12 用户要求）

**任何"大方案"（新增链路/重构/多文件改动/跨系统集成）执行完，收尾时一律先复盘、再沉淀，然后才算完成。**

**什么算复盘（四问）：**
1. **做了什么**：最终交付了什么（文件/commit/文档），哪些是验证过的、哪些只是"写了没验"。
2. **代价**：花了多少（turn/step/工具调用、token 与粗估花费、耗时），哪一步最贵。
3. **哪里可以更好**：流程层面的可优化点，按收益排序，要具体到"下次遇到同类情况该怎么做"。
4. **踩了什么坑**：每个错误记「现象 / 根因 / 修复 / 教训」，被否决的方案也要记（含为什么否决）。

**沉淀到哪（按内容分流，不要混放）：**

| 内容类型 | 落地文件 |
|---|---|
| 产品关键决策（功能形态、口径、版本线） | `doc/decision-log.md` |
| 工程化 / CI / 部署 / 依赖 / 环境 / 工具链 | `doc/standards/工程化与CI经验.md` |
| 项目本身的开发经验（前端、判定管线、页面跳转…） | `doc/standards/开发经验.md` |

**纪律：**
- **不许把复盘只写在聊天里**——聊天会丢，文档进 git。
- 复盘要**落在收尾同一轮**，不等用户催。
- 单轮/小改动不必强行走四问，但一旦出现新坑，仍按「现象/根因/修复/教训」记一条。
- 复盘要**诚实标注未验证项**（例如"密钥未到位，真实上传未验证"），不得把"写完"说成"验完"。
