...
$$ \mathbf{Q} = \left\{ x \in \mathbf{R} \mid x = \frac{p}{q},\ p, q \in \mathbf{Z},\ p \neq 0 \right\}. $$
...
2. 用符号"$\in$"或"$\notin$"填空：
   $$ 0 \ \underline{\qquad}\ \mathbf{N};\quad -3 \ \underline{\qquad}\ \mathbf{N};\quad 0.5 \ \underline{\qquad}\ \mathbf{Z};\quad \sqrt{2} \ \underline{\qquad}\ \mathbf{Z};\quad \frac{1}{3} \ \underline{\qquad}\ \mathbf{Q};\quad \pi \ \underline{\qquad}\ \mathbf{R}. $$
...
1. 用符号"$\in$"或"$\notin$"填空：
   1. 设 $A$ 为所有亚洲国家组成的集合，则
      $$ \text{中国 } \underline{\qquad}\ A,\quad \text{美国 } \underline{\qquad}\ A,\quad \text{印度 } \underline{\qquad}\ A,\quad \text{英国 } \underline{\qquad}\ A; $$
   2. 若 $A = \{ x \mid x^2 = x \}$，则 $-1 \ \underline{\qquad}\ A$；
   3. 若 $B = \{ x \mid x^2 + x - 6 = 0 \}$，则 $3 \ \underline{\qquad}\ B$；
   4. 若 $C = \{ x \in \mathbf{N} \mid 1 \le x \le 10 \}$，则 $8 \ \underline{\qquad}\ C$，$9.1 \ \underline{\qquad}\ C$.
...
$$ (2)\ C \text{ 为立德中学高一（2）班全体女生组成的集合，} D \text{ 为这个班全体学生组成的集合；} $$
...
1. $A = \{ 1, 2, 3 \}$，$B = \{ x \mid x \text{ 是 } 8 \text{ 的约数} \}$；
...
2. 用适当的符号填空：
   1. $a \ \underline{\qquad}\ \{ a, b, c \}$；
   2. $0 \ \underline{\qquad}\ \{ x \mid x^2 = 0 \}$；
   3. $\varnothing \ \underline{\qquad}\ \{ x \in \mathbf{R} \mid x^2 + 1 = 0 \}$；
   4. $\{ 0, 1 \} \ \underline{\qquad}\ \mathbf{N}$；
   5. $\{ 0 \} \ \underline{\qquad}\ \{ x \mid x^2 = x \}$；
   6. $\{ 2, 1 \} \ \underline{\qquad}\ \{ x \mid x^2 - 3x + 2 = 0 \}$.

3. 判断下列两个集合之间的关系：
   1. $A = \{ x \mid x < 0 \}$，$B = \{ x \mid x < 1 \}$；
   2. $A = \{ x \mid x = 3k, k \in \mathbf{N} \}$，$B = \{ x \mid x = 6z, z \in \mathbf{N} \}$；
   3. $A = \{ x \in \mathbf{N}_+ \mid x \text{ 是 } 4 \text{ 与 } 10 \text{ 的公倍数} \}$，$B = \{ x \mid x = 20m, m \in \mathbf{N}_+ \}$.
...
1. 选用适当的符号填空：
   1. 若集合 $A = \{ x \mid 2x - 3 < 3x \}$，$B = \{ x \mid x \ge 2 \}$，则
      $$ -4 \ \underline{\qquad}\ B,\quad -3 \ \underline{\qquad}\ A,\quad \{ 2 \} \ \underline{\qquad}\ B,\quad B \ \underline{\qquad}\ A; $$
   2. 若集合 $A = \{ x \mid x^2 - 1 = 0 \}$，则
      $$ 1 \ \underline{\qquad}\ A,\quad \{ -1 \} \ \underline{\qquad}\ A,\quad \varnothing \ \underline{\qquad}\ A,\quad \{ 1, -1 \} \ \underline{\qquad}\ A; $$
   3. $\{ x \mid x \text{ 是菱形} \} \ \underline{\qquad}\ \{ x \mid x \text{ 是平行四边形} \}$；
   4. $\{ x \mid x \text{ 是等腰三角形} \} \ \underline{\qquad}\ \{ x \mid x \text{ 是等边三角形} \}$.
...
4. 在平面直角坐标系中，集合 $C = \{ (x, y) \mid y = x \}$ 表示直线 $y = x$，从这个角度看，集合 $D = \left\{ (x, y) \mid \begin{cases} 2x - y = 1 \\ x + 4y = 5 \end{cases} \right\}$ 表示什么？集合 $C$，$D$ 之间有什么关系？
...
这样，在问题 (1)(2) 中，集合 $A$ 与 $B$ 的并集是 $C$，即 $A \cup B = C$.
...
$$ A \cup B = \{ 4, 5, 6, 8 \} \cup \{ 3, 5, 7, 8 \} = \{ 3, 4, 5, 6, 7, 8 \}. $$
...
$$ A \cup B = \{ x \mid -1 < x < 2 \} \cup \{ x \mid 1 < x < 3 \} = \{ x \mid -1 < x < 3 \}. $$
