...
**当前状态**：方案待定中（见下方决策记录）
**候选方案**：
- ❌ ~~MinerU~~（v1.2尝试，实测失败，已否掉）
- 🟡 云 OCR + AI 版面分析（腾讯云/阿里云，待测试）
- 🟡 pdf.js 直抽（仅适用于数字排版PDF，本教材为图片型不适用）
...
**输入**：阶段一的 `raw_textbook.md`（整本教材）  
**工具**：Node.js 脚本，根据 Markdown 的 `##` / `###` 标题层级 + 目录 JSON 按节切割  
**说明**：阶段二需要"逐节跑"（每节调用一次大模型），所以先把整本教材按节拆成独立文件  
**产出**：每节一个独立 Markdown 文件，存放在 `sections/` 目录
...
**拆分逻辑**：
- 读取 `raw_textbook.json` 中的 `toc` 目录结构
- 根据章节标题定位到 Markdown 中的对应段落
- 将该节内容（标题 + 正文 + 公式 + 表格 + 图片引用）提取为独立文件
- 保留元数据（学科、学段、年级、章节编号）

**产出格式示例**（`sections/1.1_正数和负数.md`）：
```markdown
# 1.1 正数和负数
...
**工具**：Node.js 脚本 + 大模型 API（`deepseek-v4-flash` / GPT），**逐节跑**
...
| 阶段 | 工具/方式 | 产出 | 策略参考 |
|------|----------|------|---------|
| 1. PDF解析 | Python `mineru`（opendatalab/MinerU） | `raw_textbook.md` + `images/` | hybrid-engine 加速（Apple Silicon） |
| 1.5 按节拆分 | Node.js 脚本（按 TOC 切割 Markdown） | `sections/*.md` | 为"逐节跑"做准备 |
| 2. 知识点提取 | Node.js 脚本 + 大模型 API `deepseek-v4-flash`（逐节） | `candidates/*.json` | 两问法、三层候选 |
| 3. 关系构建 | Node.js 脚本 + 大模型 API（逐节） | `relations/*.json` | 演化/结构双脉络思维 |
| 4. 格式化输出 | Node.js 脚本（name→ID 映射） | `final/*_knowledge_graph.json` | — |
| 5. 质量校验 | Node.js 校验脚本 | `quality_report.json` | 质量门禁框架 |
...
| 2026-07-28 | **v1.3 更新**：MinerU 在 Mac 上因 MPS 兼容性 + 网络下载问题无法正常使用，正式**否掉 MinerU**。改为**云 OCR + AI 版面分析**方案。详见下方决策记录 | MinerU 在 Mac 上跑不通（网络/兼容性问题），速度也太慢 |
