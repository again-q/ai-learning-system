// D4 原子工具：掌握度更新（K 知识点粒度 + A 单元级 + mastery_logs）
// 逐字搬迁自 cloudfunctions/judgeOne/updateMastery.js（该文件本身已是纯 DI，无任何 require，故零改动）
// 调用契约：updateMastery({ db, matchKnowledgeNode, findNode, unitNameOf, openid, questionId, question, raw, clamped })
// 返回 { mainNodeId, pOk }；失败不抛（降级不阻塞主流程）
