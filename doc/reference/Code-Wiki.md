...
本项目的核心理论来源于 `五维能力向量框架-理论文档.md` v3.0，用一个五维向量刻画学科掌握程度：

```
M = (K, A, T, Q, S)
```

| 维度 | 名称 | 认知本质 | 粒度 | 数据源 | 覆盖率 |
|------|------|---------|------|--------|--------|
| K | 知识掌握 | 陈述性记忆 | 知识点（树状聚合） | 难度×过程分（加权得分法） | 稀疏 |
| A | 应用能力 | 程序性知识流畅度 | 教材单元 | D×η（有效刺激量），A/U框架 | 全量 |
| T | 迁移能力 | 跨单元结构识别 | 全局 | W×D×r，T/U框架 | 稀疏 |
| Q | 思维品质 | 元认知（觉察与调整） | 全局 | 思维链（弹窗触发，独占） | 稀疏 |
| S | 执行稳定 | 失常模式诊断 | 全局 | S0自动计算 + S1~S6回溯自报 | 全量 |
...
- **算法**：加权得分法 Sₖ/Dₖ
  - 每道题贡献 = 难度(D) × 过程分(P)
  - 掌握度 = Σ(D×P) / Σ D
- **难度 D**：LLM 用 5 维度（知识复杂度、思维深度、陷阱密度、计算强度、陌生度）加权平均，范围 0.01~0.99
- **过程分 P**：LLM 判定 4 档（1.0/0.5/0.3/0）
...
- **算法**：W×D×r，T/U 框架
  ...
  - T_shown 与当前 T 的预期偏差驱动更新
- **粒度**：全局，每学生一个
- **初始值**：T₀=0.10, TU₀=0.60

### Q — 思维品质

- **本质**：Flavell 元认知（觉察与调整）
- **5 个子维度**：Q1 事中监控 / Q2 归因准确度 / Q3 调整清晰度 / Q4 反思深度 / Q5 执行结果
- **数据源**：思维链（弹窗触发，独占，不接受任何代理指标）
- **聚合公式**：Q = 0.25×Q1 + 0.25×Q2 + 0.25×Q3 + 0.15×Q4 + 0.10×Q5

### S — 执行稳定性

- **本质**：实际表现与真实能力的偏差
- **7 个子维度**：S0 波动指数（客观）/ S1~S4 失常模式（主观）/ S5~S6 状态归因（不计入总分）
- **S0 算法**：预期得分率 P_exp = w_K×K + w_A×A + w_T×T，d = |P_exp - R_actual|
- **S_global** = 0.6×Self_stability + 0.4×Obj_stability

该框架在 `miniprogram/pages/graph/graph.js` 的 `dimensions` 数据中落地为五个维度的可视化节点。
...
| `kOverall` | number | K 维度总分（知识掌握，加权得分法 Sₖ/Dₖ） |
| `aOverall` | number | A 维度总分（应用能力，A/U框架 + η） |
| `tOverall` | number | T 维度总分（迁移能力，W×D×r + T/U框架） |
| `sOverall` | number | S 维度总分（执行稳定，S0波动+S1~S6失常） |
| `qOverall` | number | Q 维度总分（思维品质，Flavell元认知5子维度） |
...
| 五维能力数据 | graph 页硬编码 | 接入真实五维数据（加权得分法 Sₖ/Dₖ / A/U框架 / W×D×r 等） |
