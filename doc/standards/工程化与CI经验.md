...
## 十、触发 workflow 的正确姿势：空提交不触发（2026-09-13）

- **坑 15｜`git commit --allow-empty` 不会创建 workflow 运行**：为避免误触发部署，先推了带 `[skip ci]` 的修复提交（✅ 生效，运行总数不变）；随后想"只跑一次构建"验证真机效果，于是推了一个**空提交**（消息不含任何 skip 关键字）。结果**运行没有被创建**。已排除的三个可能：
  | 排查项 | 实测结果 |
  |---|---|
  | 推送是否送达 | ✅ `PushEvent refs/heads/main head=c603468` 已出现在仓库事件流 |
  | workflow 是否被禁用 | ✅ `GET /actions/workflows` → `state=active` |
  | 平台是否异常 | ✅ `githubstatus` → Actions `operational` |
  | 唯一变量 | **提交是空的（无任何文件改动）** |
- **结论/用法**：
  - 想"验证一次构建/只跑一次"：**必须有真实文件改动**（哪怕改一行文档），空提交不行。
  - 想"不触发"：在**该次推送的提交消息**里写 `[skip ci]`（`[ci skip]` / `[no ci]` / `[skip actions]` 同理），实测对**推送的那个提交**有效。
  - 更干净的手动触发是 `workflow_dispatch`，但本机没有 `gh` CLI、也没有 GitHub token → **走不了 API**，只能靠 push 触发（见 §二 附注）。
- **教训**：排查"为什么没触发"时，用**二分法把变量收敛到唯一项**（推送事件 / workflow state / 平台状态 / 提交内容），别一上来就怀疑 workflow 写错了。本次四个候选里前三个都能用只读 API 直接证伪，剩下那个就是答案。
