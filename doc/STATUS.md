...
| # | 状态项 | 当前值（2026-09-25） | 权威来源（唯一事实源） | 联动位置（改了要同步） |
|---|---|---|---|---|
...
| 7 | **判定质量** | 壳一致 ✅（0/55）；**稳定性仍未修**（P1 精细数值当精确分、P2 η 无钳制） | `doc/architecture/诊断质量审计-2026-09-19.md` | 本表、决策日志、掌握度链路-待办 |
...
| 19 | **⚠️ 图谱质量（新发现 09-25）** | 271 节点里 **`type=method` 有 89 个 = 33%**，而决策 028 的 WWH 规定**方法/套路/过程不建节点**——说明 `method` 这个类别把「操作」（合法）与「方法套路」（违规）混在一起；另实测**同一内容挂多个节点**（并集/集合的并集；集合的子集计数/集合的子集与元素计数/子集）→ **K 记账被摊薄**（决策 026 粒度方案失效） | 云端 `knowledge_nodes`；本表 #19 | 图谱治理、D6 解读、掌握度可信度 |
